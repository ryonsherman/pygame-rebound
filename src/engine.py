    ...
    def update(self, castles, projectiles):
        ...
        if self.fire_timer <= 0:
            if not my_castle["shield"]["active"] and self.aim_point is not None:
                # Fire while still rotating for sling effect — allow firing
                # within a threshold of the target rather than waiting for exact aim
                cx, cy = my_castle["center"]
                ax, ay = _clamp_aim(self.owner, self.aim_point[0], self.aim_point[1], cx, cy)
                target_angle = math.atan2(ay - cy, ax - cx)
                diff = abs(math.atan2(math.sin(target_angle - self.current_angle),
